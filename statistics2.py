...
print("{} словоформ, {} словоупотеблений, средняя длина слова {} ".format(len(words), word_count, average_word_len))


#подсчитываем частоты букв и сторим дерево оконочаний
...
print("Безусловные вероятности:\n========================\n", sorted([(k,v) for k,v in prob.items()], key=itemgetter(1), reverse=True)[:10],"...\n")
...
for posChar in cond_prob:#цикл по позициям букв в условной вероятности
    aff_len = posChar[0]
    if cond_prob[posChar] > thres_cond[aff_len]:
        thres_cond[aff_len] = cond_prob[posChar]
        maxlet[-aff_len] = posChar[1]
for aff_len in thres_cond:
        thres_cond[aff_len] *= THRESHOLD_COEFF

print("Наиболее частые буквы по позициям:\n============================\n", maxlet[-1:0:-1],"\n")

print("Пороговые вероятности по позициям:\n============================\n", thres_cond,"\n")
...
print("Верхние подмножества:\n====================\n");
for aff_len in set(map(itemgetter(0), cond_prob_sup.keys())):
    print(aff_len, "**")
    for k,v in cond_prob_sup.items():
        if k[0] == aff_len:
            print(k[1], "{:.4f}".format(v))
